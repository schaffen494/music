...
void create_MusicBar(GtkWidget* fixed) {
    ...
    gchar *label_text = g_strdup_printf("Volume: %.0f",gtk_range_get_value(GTK_RANGE(scale)));
...
static void on_sound_value_changed(GtkWidget *range, gpointer data) {
    ...
    gchar *label_text = g_strdup_printf("Volume: %.0f", sound_value);
    // Thay đổi âm lượng

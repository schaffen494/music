...
int searchBar(GtkWidget *fixed) {
        ...
        gtk_fixed_put(GTK_FIXED(fixed), search_entry, 350, 20);
        ...
        gtk_fixed_put(GTK_FIXED(fixed), vbox, 350, 20);
        ...
        if (!gtk_css_provider_load_from_path(provider,
                                             "src\\properties\\selection_menu.css",
                                             &error)) {
            g_warning("Failed to load CSS file: %s", error->message);
            g_clear_error(&error);
        }
        GtkStyleContext *context_entry = gtk_widget_get_style_context(search_entry);
        gtk_style_context_add_provider(context_entry, GTK_STYLE_PROVIDER(provider),
